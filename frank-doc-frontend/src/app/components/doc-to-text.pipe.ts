...
export class DocToTextPipe implements PipeTransform {
  ...
  transform(value: string, elements: FrankDoc['elements'] | null): SafeHtml | string {
    ...
    // value = value.replace(markdownLinkRegex, '<a target="_blank" href="$2" alt="$1">$1</a>');
  ...
  transformLink(captureGroup: string, elements: FrankDoc['elements']): string {
    ...
    return `<span data-link data-href="#/${element.fullName}" data-text="${name}"></span>`;
  }
