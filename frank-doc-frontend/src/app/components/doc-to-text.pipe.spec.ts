import { DocToTextPipe } from './doc-to-text.pipe';
...
describe('DocToTextPipe', () => {
  // let appService: AppService;
  // let domSanitizer: DomSanitizer;
  let pipe: DocToTextPipe;
  ...
  /*it('transforms markdown link to html link', () => {
    expect(pipe.transform('A [link](http://example.com) to an example website', {})).toEqual(
      'A <a target="_blank" href="http://example.com" alt="link">link</a> to an example website',
    );
  });

  it('transforms javadoc quotes to normal quotes', () => {
    expect(pipe.transform('A javadoc \\"quote\\"', {})).toEqual('A javadoc "quote"');
  });

  it('transforms javadoc invalid link to html text', () => {
    expect(pipe.transform('A javadoc {@link http://example.com link} to an example website', {})).toEqual(
      'A javadoc link to an example website',
    );
  });

  it('transforms basic javadoc valid link to html link', () => {
    const elements: FrankDoc['elements'] = {
    ...
    expect(pipe.transform('A javadoc {@link Json2XmlValidator}', elements)).toEqual(
  ...
  it('transforms javadoc link to class with label to html link', () => {
    ...
    expect(pipe.transform('{@link PipeLineSession pipeLineSession}', elements)).toEqual(
      '<a href="#/All/PipeLineSession">pipeLineSession</a>',
    );
  });

  it('transforms invalid javadoc link to class with method to html link', () => {
    const elements: FrankDoc['elements'] = {};
    expect(pipe.transform('{@link IPipe#configure()}', elements)).toEqual('IPipe.configure()');
  });
  ...
  it('transforms javadoc link to same class method to html link', () => {
    const elements: FrankDoc['elements'] = {};
    expect(pipe.transform('{@link #doPipe(Message, PipeLineSession) doPipe}', elements)).toEqual('doPipe');
  });

  it('transforms javadoc links without parentheses', () => {
    const elements: FrankDoc['elements'] = {};
    expect(
      pipe.transform('{@link #setDestinationType destinationType} = {@link DestinationType#TOPIC TOPIC}', elements),
