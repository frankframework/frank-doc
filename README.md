...
**@Reintroduce:** Java annotation meant to change the order of config children and attributes in the XSD. The default order of config children is that inherited config children go after declared config children. To have a different order, override an inherited config child setter or attribute setter and add this annotation. The inherited element child is then sorted with the declared element children. Please note that an inherited element child is also sorted with the declared element children if the setter is overridden with a new JavaDoc comment. This annotation is needed if and only if the only modification of the element child is the sort order.
