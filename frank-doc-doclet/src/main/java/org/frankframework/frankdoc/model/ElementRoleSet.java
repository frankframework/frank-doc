 ...
 * recursion within generic element options. See {@link org.frankframework.frankdoc.model}.
 * @author martijn
 *
 */
class ElementRoleSet {
	...
	void initConflicts() {
		...
		for(ElementRole role: roles) {
			...
			for (var entry: roleMembersByName.entrySet()) {
				String name = entry.getKey();
				byXsdElementName.merge(name, new HashSet<>(entry.getValue()), FrankElement::join);
			}
		}
